...
class GroupedGreedyTuner(Tuner):
  """
  In each iteration, set the edge with the least impact on accuracy to 4

  Should be initialized with best bit configuration for int16 activation
  """
  def __init__(self, space, objective, max_trials):
    super(GroupedGreedyTuner, self).__init__(space, objective, max_trials)

    self.bits = [choices[0] for choices in space.search_space]
